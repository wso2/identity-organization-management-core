 ...
 * under the License.
 */
package org.wso2.carbon.identity.organization.management.service.filter;
...
public class FilterTreeBuilder {
    ...
    public FilterTreeBuilder(String filterString) throws IOException {
        ...
        while (input.nextToken() != StreamTokenizer.TT_EOF) {
            ...
            if (input.ttype == 40) {
                ...
                if (!(input.sval.equalsIgnoreCase(OrganizationManagementConstants.Filter.AND) ||
                        input.sval.equalsIgnoreCase(OrganizationManagementConstants.Filter.OR) ||
                        input.sval.equalsIgnoreCase(OrganizationManagementConstants.Filter.NOT))) {
     ...
     */
    public Node buildTree() throws OrganizationManagementClientException {
     ...
     */
    private void expression() throws OrganizationManagementClientException {

        term();
        while (symbol.equals(OrganizationManagementConstants.Filter.OR)) {
            OperationNode or = new OperationNode(OrganizationManagementConstants.Filter.OR);
            or.setLeftNode(root);
            term();
     ...
     * We build the parser using the recursive descent parser technique.
     */
    private void term() throws OrganizationManagementClientException {

        factor();
        while (symbol.equals(OrganizationManagementConstants.Filter.AND)) {
            OperationNode and = new OperationNode(OrganizationManagementConstants.Filter.AND);
     ...
     * We build the parser using the recursive descent parser technique.
     */
    private void factor() throws OrganizationManagementClientException {

        symbol = nextSymbol();
        if (symbol.equals(OrganizationManagementConstants.Filter.NOT)) {
            OperationNode not = new OperationNode(OrganizationManagementConstants.Filter.NOT);
            factor();
            ...
            if (!(symbol.equals(")"))) {
                ...
                throw handleClientException(ERROR_CODE_INVALID_FILTER_ARGUMENT, symbol);
            }
        }
    }
    ...
    private void validateAndBuildFilterExpression(String filterString, ExpressionNode expressionNode)
        ...
        if (StringUtils.isNotBlank(filterString) && !filterString.equals("-1")) {
            ...
            if (Pattern.compile(Pattern.quote(" eq "), Pattern.CASE_INSENSITIVE).matcher(filterString).find()) {
                ...
                throw handleClientException(ERROR_CODE_UNSUPPORTED_FILTER_OPERATION, filterString);
            }
        }
    }
    ...
    private void setExpressionNodeValues(String attributeValue, String operation, String value,
        ...
        if (StringUtils.isNotBlank(attributeValue) || StringUtils.isNotBlank(operation)) {
            ...
            throw handleClientException(ERROR_CODE_EMPTY_FILTER_VALUE, attributeValue, operation);
        }
    }
